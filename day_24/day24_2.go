// Álvaro Castellano Vela 2019/01/10
...
func fight(systems Systems) (int, string) {

	var winner string
	var winnerUnits int
	var ended bool = false

	var infiniteLoopCounter int = 0
	var lastAliveImmunes, lastAliveInfections int = -1, -1
	...
	for ended != true {
		...
		if infectionAlive == 0 && ended == false {
			ended = true
			winner = "Immune"
		}

		if immuneAlive == lastAliveImmunes && infectionAlive == lastAliveInfections && ended == false {
			infiniteLoopCounter++
		} else {
			infiniteLoopCounter = 0
		}
		lastAliveImmunes = immuneAlive
		lastAliveInfections = infectionAlive
		if infiniteLoopCounter > 5000 && ended == false {
			ended = true
			winner = "Infection"
		}
	}
...
func main() {
	...
	for winner != "Immune" {
		...
		for id, _ := range systems["Immune"] {
			systems["Immune"][id].Damage += boost
			systems["Immune"][id].EffectivePower = systems["Immune"][id].Units * systems["Immune"][id].Damage
		}
		result, winner = fight(systems)
		boost += increment
	}
